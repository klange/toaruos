...
static void print_entry(struct tfile * file, int *colwidth) {
	...
	const char * ansi_color_str = ls_color_str(file->name, &file->statbuf);
	...
	if (S_ISLNK(file->statbuf.st_mode)) {
		if (file->lstatres && LS_C(ORPHAN)) {
			...
			ansi_color_str = ls_color_str(file->link, &file->statbufl);
		}
	}
...
static int display_dir(char * p) {
	...
	display_tfiles(file_arr, index, long_mode || show_size); /* || a few other things as well we don't have yet */
